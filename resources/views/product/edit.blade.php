            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <small class="text-muted">Slug akan otomatis di-generate dari title.</small>
                    </div>
                    ...
                    {{-- Why Points (dynamic) --}}
                    @php
                        $whyPoints = json_decode($product->why_points ?? '[]', true) ?: [];
                    @endphp
                    <div class="col-12 mb-3">
                        <label class="form-label">Why Points</label>
                        <div id="why-points-container">
                            @forelse($whyPoints as $point)
                                <div class="input-group mb-2 why-point-row">
                                    <input type="text" name="why_points[]" class="form-control" value="{{ $point }}">
            ...
            <div class="card-body">
                <div id="details-container">
                    @foreach($details as $idx => $detail)
                        @php
                            $detailImages = json_decode($detail->image ?? '[]', true) ?: [];
                        @endphp
                        <div class="detail-card" id="detail-card-{{ $idx }}">
                            <button type="button" class="btn btn-sm btn-danger btn-remove-detail" onclick="removeDetail({{ $idx }})">
                                <i class="bi bi-x-lg"></i>
                            </button>
                            <h6 class="mb-3">Detail #{{ $idx + 1 }}</h6>
                            <input type="hidden" name="detail_id[{{ $idx }}]" value="{{ $detail->id }}">
                            <div class="row">
                                <div class="col-md-8 mb-3">
                                    <label class="form-label">Title</label>
                                    <input type="text" name="detail_title[{{ $idx }}]" class="form-control" value="{{ $detail->title }}" required>
                                </div>
                                <div class="col-md-4 mb-3">
                                    <label class="form-label">Display Order</label>
                                    <input type="number" name="detail_display_order[{{ $idx }}]" class="form-control" value="{{ $detail->display_order }}" min="0">
                                </div>
                                <div class="col-12 mb-3">
                                    <label class="form-label">Description <small class="text-muted">(CKEditor)</small></label>
                                    <textarea name="detail_description[{{ $idx }}]" id="detail-desc-{{ $idx }}" class="form-control ckeditor-init" rows="5">{{ $detail->description }}</textarea>
                                </div>
                                <div class="col-12 mb-3">
                                    ...
                                    @if(count($detailImages) > 0)
                                        <div class="mb-2 d-flex flex-wrap gap-2">
                                            ...
                                            @endforeach
                                            <small class="d-block text-muted w-100">Gambar saat ini (upload baru akan mengganti semua)</small>
                                        </div>
                                    @endif
                                    <div id="detail-images-container-{{ $idx }}">
                                        <div class="detail-image-row">
                                            <input type="file" name="detail_images[{{ $idx }}][]" class="form-control" accept=".webp">
                                            <button type="button" class="btn btn-sm btn-danger" onclick="removeDetailImage(this)">
                                                <i class="bi bi-x-lg"></i>
                                            </button>
                                        </div>
                                    </div>
                                    <button type="button" class="btn btn-sm btn-outline-success mt-1" onclick="addDetailImage({{ $idx }})">
                ...
                <div id="no-detail-msg" class="text-center text-muted py-3" @if(count($details) > 0) style="display:none" @endif>
...
@push('js')
    ...
    <script>
        ...
        // Initialize CKEditor for existing details on page load
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.ckeditor-init').forEach(function(el) {
                ClassicEditor
                    .create(el, {
        ...
        function addDetail() {
            ...
            const html = `
                <div class="detail-card" id="detail-card-${detailIndex}">
                    ...
                    <input type="hidden" name="detail_id[${detailIndex}]" value="">
                    <div class="row">
                        <div class="col-md-8 mb-3">
                            <label class="form-label">Title</label>
            ...
            // Initialize CKEditor for this new detail
            ClassicEditor
